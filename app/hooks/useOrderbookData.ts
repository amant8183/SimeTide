...
function transformWebSocketMessage(message: any, venue: Venue): OrderbookData | null {
  try {
    let rawBids: any[] = [];
    let rawAsks: any[] = [];
    
    if (venue === 'OKX' && message.arg?.channel === 'books' && message.data?.[0]) {
      rawBids = message.data[0].bids || [];
      rawAsks = message.data[0].asks || [];
    } else if (venue === 'Bybit' && message.topic?.includes('orderbook') && message.data) {
